...
def copy_role_tags_to_customer_managed_policy(event, context):
     ...
     * **Permissions:** This function requires extensive IAM permissions, including:
        * `iam:ListPolicies`
        ...
        * `iam:ListRoleTags`
        * `iam:TagPolicy`
        * `iam:UntagPolicy` (if you want to remove existing tags)
    """
...
# Example invocation (for local testing - remember to configure AWS credentials)

if __name__ == "__main__":
    # Replace with a dummy event and context if needed for local testing
    event = {}
    context = None
    copy_role_tags_to_customer_managed_policy(event, context)